...
import { Wallet, ChevronRight, ChevronLeft, CheckCircle, Zap, Lock, ArrowRight, ExternalLink } from 'lucide-react';
...
const heroCards = [
  {
    ...
    meter: '92%',
    description: 'Tu perfil on-chain cumple los parámetros del pool institucional.',
  },
  {
    label: 'Estado',
    title: 'Score en revisión',
    meter: '76%',
    description: 'Algunas métricas pueden mejorar para acceder a mejores tasas.',
  },
  {
    label: 'Estado',
...
const testimonials = [
  {
    ...
    quote: 'Filtramos riesgo sin pedir KYC completo. Bajó la fricción y subió la calidad.',
  },
  {
    name: 'Desk institucional',
    quote: 'Ofrecemos crédito con parámetros claros sin ver balances de usuario.',
  },
  {
    name: 'Builder anónimo',
    quote: 'La reputación financiera privada es el puente real entre DeFi y TradFi.',
  },
];
        ...
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-16 relative z-10">
          <div className="space-y-8 hero-intro">
            ...
            <p className="hero-item hero-item-3 text-lg text-blue-100/80 max-w-lg leading-relaxed border-l-2 border-blue-400/20 pl-6">
              PrivyCredit genera pruebas selladas en Scroll. Demuestras solvencia on-chain con Zero-Knowledge sin exponer
              montos ni contrapartes.
            </p>
            <div className="hero-item hero-item-4 flex flex-col sm:flex-row gap-4 pt-2">
              <Button onClick={() => setCurrentScreen('connect')} className="!text-base !px-8 !py-4 shadow-blue-900/20">
                Probar ahora
                <ArrowRight className="w-5 h-5 transition-transform" />
              </Button>
              <Button variant="ghost" onClick={() => setCurrentScreen('verifier')} className="!px-6">
            ...
            <div className="hero-item hero-item-5 pt-8 border-t border-white/5 space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                {[
                  { label: 'Tiempo promedio', value: '30s', detail: 'Generar prueba' },
                  { label: 'Privacidad', value: '100%', detail: 'Montos sellados' },
                  { label: 'Validez', value: '30 días', detail: 'Renovable' },
                ].map((metric) => (
                  <Card key={metric.label} className="!bg-[#0b1222]/70 border-white/5 shadow-none">
                    <p className="text-[11px] uppercase tracking-[0.3em] text-blue-200/50 mb-2">{metric.label}</p>
              ...
              <div>
                ...
                <div className="logo-marquee-container">
                  <div className="logo-marquee-track">
                    {['Scroll', 'Privy', 'Chainlink', 'ZK Stack'].map((partner) => (
                      <div
                        key={partner}
          ...
          <div className="relative hidden lg:block perspective-[2000px]">
            <div className="relative z-10 transform transition-transform duration-700 hover:rotate-y-[-2deg] hover:rotate-x-[2deg] hero-big-card">
              ...
              <Card noPadding className="overflow-hidden aspect-[4/3] group cursor-default !bg-[#0f172a]/80 card-rotate">
                <div className="absolute inset-0 bg-gradient-to-b from-transparent via-[#0f172a]/20 to-[#0f172a]/80 z-10 pointer-events-none" />
                <div className="w-full h-full relative p-8 flex flex-col items-center justify-center bg-cover bg-center opacity-90 group-hover:scale-105 transition-transform duration-1000" style={{ backgroundImage: `url(${heroMock})` }}>
                  <div className="absolute bottom-12 right-12 bg-[#1e293b]/90 backdrop-blur-xl border border-white/10 p-4 rounded-xl shadow-2xl animate-float hero-card-content max-w-xs">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <div className="text-xs text-slate-400">{heroCards[activeCard].label}</div>
                        <div className="text-sm font-bold text-white">{heroCards[activeCard].title}</div>
                      </div>
                    </div>
                    <p className="text-[11px] text-slate-300/80 mb-4">{heroCards[activeCard].description}</p>
                    <div className="h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
                      <div className="h-full rounded-full bg-emerald-500 relative" style={{ width: heroCards[activeCard].meter }}>
                  ...
                  <div className="absolute bottom-6 left-6 z-20 flex gap-2">
                    <button
                      onClick={() => setActiveCard((prev) => (prev === 0 ? heroCards.length - 1 : prev - 1))}
      ...
      <section className="py-20 relative border-y border-white/5 bg-black/10 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-12">
          <div className="text-center max-w-2xl mx-auto space-y-4">
            ...
            <h2 className="text-3xl md:text-4xl font-bold text-white">Privacidad en 3 pasos</h2>
            <p className="text-blue-100/70">
              El prestamista conoce que eres solvente, pero no tiene acceso a tus cifras exactas. Todo sucede dentro de tu
              dispositivo.
            </p>
          </div>
          <div className="grid md:grid-cols-3 gap-8 relative">
            ...
            {processSteps.map((item) => (
              <div key={item.step} className="relative z-10 group text-center px-4">
      ...
      <section className="max-w-7xl mx-auto w-full">
        <div className="bg-gradient-to-br from-indigo-900/20 to-blue-900/10 rounded-3xl border border-white/10 p-8 md:p-12 flex flex-col md:flex-row items-center gap-12 backdrop-blur-md shadow-2xl">
          <div className="flex-1 space-y-6">
            <h3 className="text-2xl font-bold text-white">Simulador de Crédito</h3>
            <p className="text-blue-100/70">
              Prueba escenarios antes de generar una solicitud real y entiende cómo tus hábitos impactan tu banda.
            </p>
            <ul className="space-y-3">
              {['Análisis de saldo promedio', 'Historial de liquidaciones', 'Antigüedad de la wallet'].map((feat) => (
                <li key={feat} className="flex items-center gap-2 text-sm text-blue-200/80">
                  <CheckCircle className="w-4 h-4 text-blue-400" />
                  {feat}
                </li>
              ))}
            </ul>
            <Button onClick={() => setCurrentScreen('simulator')} className="!px-6 !py-3 !rounded-2xl w-fit">
              Abrir simulador
            </Button>
          </div>
      ...
      <section className="py-20 border-t border-white/5 bg-black/20 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 space-y-6 text-center">
          ...
          <p className="text-blue-100/70 max-w-2xl mx-auto text-sm md:text-base">
            Protocolos institucionales y equipos DeFi integran solvencia privada para mejorar onboarding.
          </p>
        </div>
          ...
          <div className="testimonial-track">
            {[...testimonials, ...testimonials].map((t, idx) => (
              ...
              >
                ...
                <span className="text-xs font-semibold text-blue-300">{t.name}</span>
              </Card>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-5xl mx-auto w-full">
        <Card className="p-8 md:p-10 !bg-[#030712]/70 border-white/10 flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1 space-y-3">
            <p className="text-xs uppercase tracking-[0.5em] text-blue-200/60">Contrato verificado</p>
            <h3 className="text-2xl font-semibold text-white">Scroll · {SCROLL_SEPOLIA_NAME}</h3>
            <p className="text-sm text-blue-100/70">
              Todas las pruebas se anclan en nuestro smart contract. Puedes revisarlo en el explorador cuando quieras.
            </p>
          </div>
